...
if T4ClapCalTimer6.Value < 4 then  // Условие пуска звукового оповещения
 ...
 T4ClapLatchStatusByte6.Value := 4; // Устанавливаем статус задвижки в "Калибровка"
 ...
 begin
T4ClapCloseStatus6.Value := True; // Запускаем алгоритм закрытия задвижки
AddMessage(Now, mkWarning , 'Клапан Транспортера 4 не в позиции Силос 16! Повторите команду калибровки после автоматического закрытия задвижки!', True, True);
T4ClapCalStatus6.Value := False; // Останавливаем алгоритм калибровки
...
T4ClapLatchStatusByte6.Value := 3;      // Устанавливаем статус задвижки в "Закрыта"
...
if T4ClapCalTimer6.Value > 600 then // предельное время калибровки, 10 мин
begin
T4ClapLatchRunOpen.Value := False;
T4ClapLatchRunClose.Value := False;
T4ClapBlockSelectStatus.Value := False;
T4ClapLatchStatusByte6.Value := 5;
T4ClapCalStatus6.Value := False;
