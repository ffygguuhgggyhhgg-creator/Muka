...
if T1CalTimer8.Value < 4 then  // Условие пуска звукового оповещения
 ...
 T1LatchStatusByte8.Value := 4; // Устанавливаем статус задвижки в "Калибровка"
 ...
 begin
T1CloseStatus8.Value := True; // Запускаем алгоритм закрытия задвижки
AddMessage(Now, mkWarning , 'Задвижка Транспортера 3 маршрут Т3-Т7 не закрыта! Повторите команду калибровки после автоматического закрытия задвижки!', True, True);
...
T1LatchStatusByte8.Value := 3;      // Устанавливаем статус задвижки в "Закрыта"
T1CalStatus8.Value := False; // Останавливаем выполнение скрипта
...
if T1CalTimer8.Value > 600 then // предельное время калибровки, 10 мин
begin
T1LatchRunOpen.Value := False;
T1LatchRunClose.Value := False;
T1BlockSelectStatus.Value := False;
T1LatchStatusByte8.Value := 5;
T1CalStatus8.Value := False;
T1LatchSelect8.Value := False;
T1CalTimer8.Value := 0;
