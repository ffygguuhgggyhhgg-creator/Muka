...
if T7CalTimer3.Value < 4 then  // Условие пуска звукового оповещения
 ...
 T7LatchStatusByte3.Value := 4; // Устанавливаем статус задвижки в "Калибровка"
 ...
 begin
T7CloseStatus3.Value := True; // Запускаем алгоритм закрытия задвижки
AddMessage(Now, mkWarning , 'Задвижка Транспортера 7 Силос 53 не закрыта! Повторите команду калибровки после автоматического закрытия задвижки!', True, True);
T7CalStatus3.Value := False; // Останавливаем алгоритм калибровки
...
T7LatchStatusByte3.Value := 3;      // Устанавливаем статус задвижки в "Закрыта"
...
if T7CalTimer3.Value > 600 then // предельное время калибровки, 10 мин
begin
T7LatchRunOpen.Value := False;
T7LatchRunClose.Value := False;
T7BlockSelectStatus.Value := False;
T7LatchStatusByte3.Value := 5;
T7CalStatus3.Value := False;
