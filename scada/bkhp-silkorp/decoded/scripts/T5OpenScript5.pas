begin
if T5OpenStatus5.AsBool = True then //Если выполнен скрипт по нажатии на кнопку управления
begin
T5OpenTimer5.Value := T5OpenTimer5.Value + 1; //Запускаем счетчик выполнения алгоритма

if T5OpenTimer5.Value > 2 then  // Условие пуска звукового оповещения
if T5OpenTimer5.Value < 4 then  // Условие пуска звукового оповещения
 begin
...
if T5OpenTimer5.Value > 5 then // Условие пуска исполнительного механизма
if T5OpenTimer5.Value < 7 then
begin
...
T5OpenStatus5.Value := False; // Останавливаем выполнение скрипта
T5LatchSelect5.Value := False; // Снимаем выбор задвижки
T5OpenTimer5.Value := 0; //Останавливаем счетчик выполнения алгоритма
AddMessage(Now, mkWarning , 'Задвижка Транспортера 5 Силос 27-37 успешно открыта!', True, True);
end;

if T5OpenTimer5.Value > 9 + T5LatchCalOpenTime5.Value then // Условие остановки выполнения алгоритма по времени
begin
T5LatchRunOpen.Value := False;   // Дискретный выход управления задвижкой
T5BlockSelectStatus.Value := False;  // Снимаем блокировку управления
