begin
if T5OpenStatus4.AsBool = True then //Если выполнен скрипт по нажатии на кнопку управления
begin
T5OpenTimer4.Value := T5OpenTimer4.Value + 1; //Запускаем счетчик выполнения алгоритма

if T5OpenTimer4.Value > 2 then  // Условие пуска звукового оповещения
if T5OpenTimer4.Value < 4 then  // Условие пуска звукового оповещения
 begin
...
if T5OpenTimer4.Value > 5 then // Условие пуска исполнительного механизма
if T5OpenTimer4.Value < 7 then
begin
...
T5OpenStatus4.Value := False; // Останавливаем выполнение скрипта
T5LatchSelect4.Value := False; // Снимаем выбор задвижки
T5OpenTimer4.Value := 0; //Останавливаем счетчик выполнения алгоритма
AddMessage(Now, mkWarning , 'Задвижка Транспортера 5 Силос 26-36 успешно открыта!', True, True);
end;

if T5OpenTimer4.Value > 9 + T5LatchCalOpenTime4.Value then // Условие остановки выполнения алгоритма по времени
begin
T5LatchRunOpen.Value := False;   // Дискретный выход управления задвижкой
T5BlockSelectStatus.Value := False;  // Снимаем блокировку управления
