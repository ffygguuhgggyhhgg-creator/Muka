begin
if T5ClapOpenStatus4.AsBool = True then //Если выполнен скрипт по нажатии на кнопку управления
begin
T5ClapOpenTimer4.Value := T5ClapOpenTimer4.Value + 1; //Запускаем счетчик выполнения алгоритма
if T5ClapOpenTimer4.Value > 2 then  // Условие пуска звукового оповещения
if T5ClapOpenTimer4.Value < 4 then  // Условие пуска звукового оповещения
 begin
...
if T5ClapOpenTimer4.Value > 5 then // Условие пуска исполнительного механизма
if T5ClapOpenTimer4.Value < 7 then
begin
...
T5ClapOpenTimer4.Value := 0; //Останавливаем счетчик выполнения алгоритма
AddMessage(Now, mkWarning , 'Клапан Транспортера 5 в Силос 26 успешно переброшен!', True, True);
end;                                                                                 
                            
if T5ClapOpenTimer4.Value > 9 + T5ClapLatchCalOpenTime4.Value then // Условие остановки выполнения алгоритма по времени
begin
T5ClapLatchRunOpen.Value := False;   // Дискретный выход управления задвижкой
T5ClapBlockSelectStatus.Value := False;  // Снимаем блокировку управления
